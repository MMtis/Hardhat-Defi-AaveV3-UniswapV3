...
async function main() {
    ...
    const amountDaiToBorrow =
        (availableBorrowsBase.toString() / Math.pow(10, priceETHUSDDecimals.toString())) *
        0.95 *
    ...
    await borrowDai(daiTokenAddress, Pool, amountDaiToBorrowWei, isStableRateEnabled, deployer)
    ...
    await repay(amountDaiToBorrowWei, daiTokenAddress, Pool, isStableRateEnabled, deployer)
    await getBorrowUserData(Pool, deployer)
    console.log("*****************************************************************************")
    ...
    if (routerExists) {
        ...
        const amountLeft =
            (totalDebtBase.toString() / Math.pow(10, priceETHUSDDecimals.toString())) *
            (Math.pow(10, priceDAIUSDDecimals.toString()) / priceDAIUSD.toNumber())
        ...
        await repay(amountLeftWei, daiTokenAddress, Pool, isStableRateEnabled, deployer)
        console.log("*****************************************************************************")
...
async function swapWEthToDai(WETH, DAI, amount, account) {
...
async function repay(amount, daiAddress, Pool, isStableRateEnabled, account) {
...
async function borrowDai(daiAddress, Pool, amountDaiToBorrowWei, isStableRateEnabled, account) {
    let borrowTx
    let blocksToMine = network.config.chainId == 31337 ? 1 : 3
    if (isStableRateEnabled == 0) {
